...
def plan_entrenamiento(num_dias):
    print(f"Iniciando optimización con {num_dias} días.")
    ...
    # Restricción: descanso entre entrenamientos
    for grupo in grupos:
    ...
    # Restricción: entrenado binario y series
    for grupo in grupos:
        for dia in dias:
        ...
        html.Div(
            ...
            children=[
                html.Div(
                    style={'flex': '3'},
                    children=[
                        html.Div('Para comenzar ingrese el número de días disponibles para entrenar (mínimo 3, máximo 7):',
                        ...
                        html.Div(
                            ...
                            children=[
                                dcc.Textarea(id='dias', name='dias', rows='1', cols='50')
                            ]
                        ),
                        html.Button('Enviar', id='submit-button', n_clicks=0, style={'borderRadius': '15px', 'backgroundColor': '#274C77', 'color': 'white', 'marginTop': '5px', 'padding': '0 50px'}),
                        html.Div(id='resultado', className='resultado', style={'width': '600px', 'height': '300px', 'marginTop': '20px', 'marginBottom': '10px', 'padding': '20px', 'backgroundColor': '#a3cef1a6', 'boxShadow': '0 0 10px rgba(0, 0, 0, 0.1)', 'color': '#274C77', 'overflow': 'auto'}),
                        html.A('Volver', href='/', className='back', style={'backgroundColor': '#274C77', 'borderRadius': '15px', 'paddingLeft': '20px', 'paddingRight': '20px', 'color': 'white', 'textDecoration': 'none', 'marginTop': '20px'})
                    ]
                ),
                html.Div(
                    className='imagen2',
                    ...
                    children=[
                        html.Img(src='/assets/imgs/muscle 2.png', alt='ej2', style={'maxWidth': '400px', 'height': 'auto'})
                    ]
                )
            ]
        ),
...
def handleButtonClick(n_clicks, dias):
    print(f"Callback ejecutado con n_clicks={n_clicks} y dias={dias}")  # Agrega este print
    if n_clicks > 0 and dias:
        try:
            num_dias = int(dias)
            print(f"Número de días ingresados: {num_dias}")
            if num_dias >= 3 and num_dias <= 7:
                resultado = plan_entrenamiento(num_dias)
                print(f"Resultado generado: {resultado}")  # Agrega este print
